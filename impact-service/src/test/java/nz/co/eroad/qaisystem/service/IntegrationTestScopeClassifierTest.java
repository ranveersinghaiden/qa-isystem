...
/**
 * Unit tests for {@link IntegrationTestScopeClassifier}.
 *
 * The classifier identifies WHICH components need integration/E2E testing based on
 * component type — it does NOT assess whether tests exist (that is E2ECoverageAnalyzer's job).
 * Level is always UNKNOWN; untestedComponents lists components requiring integration tests.
 */
@DisplayName("IntegrationTestScopeClassifier unit tests")
class IntegrationTestScopeClassifierTest {
    IntegrationTestScopeClassifier classifier;
    @BeforeEach void setUp() { classifier = new IntegrationTestScopeClassifier(); }
    ...
    void level_alwaysUnknown() {
        CoverageReport r = classifier.assess(List.of(svc("AuthService")), List.of(src("AuthService.java")));
    ...
    void controllerComponent_apiTestRequired() {
        CoverageReport r = classifier.assess(List.of(ctrl("PaymentController")), List.of(src("PaymentController.java")));
    ...
    void repositoryComponent_integrationTestRequired() {
        CoverageReport r = classifier.assess(List.of(repo("OrderRepository")), List.of(src("OrderRepository.java")));
    ...
    void testComponent_excluded() {
        CoverageReport r = classifier.assess(List.of(tcomp("FooTest")), List.of(src("FooTest.java")));
    ...
    void modelComponent_excluded() {
        CoverageReport r = classifier.assess(List.of(model("OrderDto")), List.of(src("OrderDto.java")));
    ...
    void empty_noTests() {
        CoverageReport r = classifier.assess(List.of(), List.of());
        assertThat(r.getLevel()).isEqualTo(CoverageLevel.UNKNOWN);
    ...
    void securityDiff_addsE2eType() {
        GitDiff secDiff = GitDiff.builder()
                ...
                .hunks(List.of(GitDiff.DiffHunk.builder()
                        ...
                        .build()))
                .build();
        CoverageReport r = classifier.assess(List.of(svc("SecurityConfig")), List.of(secDiff));
    ...
    void testedComponents_alwaysEmpty() {
        CoverageReport r = classifier.assess(
