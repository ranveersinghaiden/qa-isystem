...
/**
 * Classifies which impacted components require integration or E2E test coverage,
 * based solely on their {@link ComponentType} and the nature of changes in the diff.
 *
 * <p><b>This class does NOT check the test repository and does NOT measure coverage.</b>
 * Its only job is to answer: "Given the changed components, which ones need integration
 * or E2E tests by nature of their type?" The result always carries
 * {@code level = UNKNOWN} and {@code source = UNKNOWN} because no repo scan is performed.
 *
 * <p>Real coverage assessment ({@code GOOD / PARTIAL / NONE}) is performed downstream by
 * {@code E2ECoverageAnalyzer} in strategy-service, which has access to the cloned
 * test repository and its coverage index.
 *
 * <p>Outputs:
 * <ul>
 *   <li>{@code untestedComponents} — component names whose type warrants integration testing</li>
 *   <li>{@code requiredTestTypes} — test categories needed (API, INTEGRATION, E2E)</li>
 * </ul>
 */
@Slf4j
@Service
public class IntegrationTestScopeClassifier {
    ...
    public CoverageReport assess(List<ImpactedComponent> components,
        ...
        log.info("[IntegrationTestScopeClassifier] {} components require integration/E2E tests: {} | types: {}",
