 ...
 *
 * <p>This bean is only created when {@code aiqa.github.enabled=true} (the default for
 * services that need GitHub integration — strategy, codegen, feedback).
 * Services that do <em>not</em> interact with GitHub (e.g. pr-service, impact-service)
 * should set {@code aiqa.github.enabled=false} in their {@code application.yaml} to
 * avoid unnecessary credential resolution at startup.
 */
@Slf4j
@Service
...
public class GitHubService {
    ...
    public GitHubService(TargetRepoProperties repoProps, ObjectMapper objectMapper) {
        ...
        // ── Token resolution ────────────────────────────────────────────────
        // Skip entirely when URL is not configured — no need for any credentials
        // when GitHub PR creation is disabled. This avoids interactive prompts
        // from git credential fill when TARGET_REPO_URL is blank.
        this.resolvedToken = (owner != null) ? resolveToken(repoProps) : null;
    ...
    private static String resolveFromCredentialHelper(String host) {
        try {
            ...
            // Disable interactive terminal prompts — if no credential is cached the
            // helper should fail silently rather than blocking on stdin waiting for
            // a username/password. Without this, git may prompt the terminal on
            // machines with no stored credential (e.g. CI agents, pr-service, etc.).
