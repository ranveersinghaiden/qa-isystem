 ...
 *
 * <p>The primary implementation is {@link OpenAiClient}, which calls any
 * OpenAI-compatible endpoint (OpenAI, Azure OpenAI, local Ollama, etc.).
 *
 * <p>When no API key is configured {@link #isAvailable()} returns {@code false}
 * and callers fall back to enhanced template generation that still embeds
 * the product expert context as comments in the generated output.
 */
